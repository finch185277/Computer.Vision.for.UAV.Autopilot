    ...
    for x in range(width):
        for y in range(height):
            if x == (x/r) * r and y == (y/r) * r:
                ...
                for z in range(depth):
                    ret[y, x, z] += img[int(y/r), int(x/r), z] * \
                        (int(x/r) + 1 - float(x/r)) * (int(y/r) + 1 - float(y/r))
                    ret[y, x, z] += img[int(y/r + 1), int(x/r), z] * \
                        (int(x/r) + 1 - float(x/r)) * (float(y/r) - int(y/r))
                    ret[y, x, z] += img[int(y/r), int(x/r + 1), z] *\
                        (float(x/r) - int(x/r)) * (int(y/r) + 1 - float(y/r))
                    ret[y, x, z] += img[int(y/r + 1), int(x/r + 1), z] * \
                        (float(x/r) - int(x/r)) * (float(y/r) - int(y/r))
